...
const FAMILY_META = [
  { ticker: "FTLX", icon: Trophy, accent: "text-success", accentBg: "bg-success/10 border-success/25" },
  { ticker: "FGMX", icon: Gamepad2, accent: "text-amber-400", accentBg: "bg-amber-400/10 border-amber-400/25" },
...
export function IndexFamily() {
  const t = useTranslations("indexFamily")
  ...
  const data = useMemo(() => {
    return FAMILY_META.map((f) => {
      ...
      return {
        ...f,
        title: t(`items.${f.ticker}.title`),
        tagline: t(`items.${f.ticker}.tagline`),
        description: t(`items.${f.ticker}.description`),
        nav: navNum > 0
          ? navNum.toFixed(2)
    ...
    <section className="relative w-full border-b border-border/40 py-28 sm:py-36 overflow-hidden">
      ...
      <div className="relative mt-14 sm:mt-16">
        ...
        <div className="flex animate-scroll w-max gap-5 sm:gap-6">
          {loop.map((item, i) => (
            <IndexFactCard key={`${item.ticker}-${i}`} item={item} navLabel={t("nav")} viewLabel={t("view")} constituentsSuffix={t("constituents")} />
          ))}
        </div>
      </div>
    </section>
        ...
        <div className="h-28">
          <div className="flex items-baseline gap-2">
            ...
            <span className="text-[10px] font-medium uppercase tracking-[0.14em] text-muted-foreground">
              · {item.constituents} {constituentsSuffix}
            </span>
          </div>
            ...
            <div className="flex -space-x-2">
              {visibleTokens.map((sym) => {
                ...
                return (
                  ...
                  >
                    {token?.icon ? (
                      <Image src={token.icon} alt={sym} width={26} height={26} className="rounded-full" />
                    ) : (
                      <span className="text-[9px] font-semibold text-foreground">{sym.slice(0, 3)}</span>
                    )}
                  </div>
                )
              })}
