...
export function PortfolioInfrastructure() {
  ...
  const positions = useMemo(() => {
    return SHOWCASE_TICKERS.map((ticker, i) => {
      ...
      // Synthetic "entry" so we can show realistic P&L without fake numbers in storage
        ...
        <div className="grid grid-cols-1 gap-16 lg:grid-cols-[0.9fr_1.1fr] lg:items-start">
          ...
          <div className="lg:sticky lg:top-32">
            ...
            <p className="mt-5 max-w-md text-base leading-relaxed text-muted-foreground">
              Every index position is minted as a non-fungible token, custodied on-chain. View, transfer, or redeem your indexed exposure with institutional-grade transparency.
            </p>
            ...
            <ul className="mt-8 space-y-3.5 text-sm text-muted-foreground">
              <li className="flex items-center gap-2.5">
                ...
                <span>NFT-based receipts for every indexed position</span>
              </li>
              <li className="flex items-center gap-2.5">
                <CheckCircle2 className="h-4 w-4 text-success" strokeWidth={1.75} />
                <span>Real-time NAV and P&amp;L from on-chain pricing</span>
              </li>
              <li className="flex items-center gap-2.5">
                <CheckCircle2 className="h-4 w-4 text-success" strokeWidth={1.75} />
                <span>Atomic redemption back to underlying assets</span>
              </li>
            </ul>
          ...
          <div className="overflow-hidden rounded-2xl border border-border/70 bg-card/30 backdrop-blur-sm">
            ...
            <div className="grid grid-cols-2 gap-px border-b border-border/60 bg-border/40 sm:grid-cols-4">
              <Metric
                ...
                value={`${formatCompact(portfolioValue)} CHZ`}
              />
              <Metric
                icon={Activity}
                label="Performance"
              ...
              />
              <Metric icon={ChainDot} label="Chain Status" value="Mainnet" tone="positive" />
            </div>
...
function Metric({
  ...
  return (
    <div className="bg-background/60 px-5 py-4">
      ...
      <div className={`mt-1.5 font-mono text-base font-semibold tracking-tight ${toneClass}`}>
        {value}
      </div>
    </div>
  )
